...
public class Board extends JPanel implements ActionListener {
    ...
    class Keyboard extends KeyAdapter {
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if (keys.contains(Integer.valueOf(KeyEvent.VK_ESCAPE))
                ...
                // pause the game if it's not paused and resume if it is
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // check if the pacman collides with any of the ghosts
        for (Ghost ghost : ghosts) {
            // pause the ghosts for few seconds after half of the food is eaten
            if (score > numOfFood / 2) {
                countPauseTime++;
            }
            if (countPauseTime > 300 || countPauseTime == 0) {
                ghost.moveRandom();
            }
            // check collides between pacman and the ghosts
            if (status == "game") {
            ...
            if (status == "game2") {
                if (pacman.checkCollide(ghost.getPosX(), ghost.getPosY())&&!stopImage) {
                    livesLeft--;
                    pacman = new Pacman(345, 10);
                    // reset the position of the ghost after collision
                    ghosts.set(ghosts.indexOf(ghost),
                ...
                if (pacman2.checkCollide(ghost.getPosX(), ghost.getPosY())&&!stopImage2) {
                    livesLeft2--;
                    pacman2 = new Pacman(10, 10);
                    // reset the position of the ghost after collision
                    ghosts.set(ghosts.indexOf(ghost),
